...
import src.vis.saffine.detrending_method as dm
...
def adaptiveline(x1, x2, dates,
                 x1_lab, x2_lab,
                 c=["lightsteelblue", "royalblue", "midnightblue"],
                 filter_span=[128, 56, 32],
    ...
    ax[0].plot(dates, normalize(x1, lower=0),c="gray")
    for i, span in enumerate(filter_span):
        n_smooth = normalize(adaptive_filter(x1, span=span), lower=0)
        ax[0].plot(dates, n_smooth,c=c[i])
    ax[0].set_ylabel(x1_lab, fontsize=14)
    
    ax[1].plot(dates, normalize(x2, lower=0),c="gray")
    for i, span in enumerate(filter_span):
        r_smooth = normalize(adaptive_filter(x2, span=span), lower=0)
